...
import { Camera, FileText, Users, DollarSign, Eye, Edit3, Check, Clock, Package, Mail, Calendar, Star, Gift, UserPlus, ExternalLink, Settings, Send, Download, Phone, MapPin, Globe, Zap } from 'lucide-react';
...
You've just reached {{tier}} membership status with {{loyaltyPoints}} loyalty points!
...
Ready to redeem your points? You can use them towards your next service or save up for a FREE sharpening!

Keep those knives coming!
...
{{businessName}}`
  ...
  const addLoyaltyPoints = (customerId, points, reason) => {
    setCustomers(prev => prev.map(customer => {
      if (customer.id === customerId) {
        ...
        if (newTier !== oldTier) {
          setTimeout(() => {
            sendEmail(customer, 'loyaltyMilestone', { tier: newTier, loyaltyPoints: newPoints });
          }, 1000);
        }
  ...
  const sendEmail = (customer, templateType, extraData = {}) => {
    ...
    if (templateType === 'loyaltyMilestone') {
      const tierBenefits = loyaltyTiers[data.tier]?.benefits || '';
      emailBody = emailBody.replace('{{tierBenefits}}', tierBenefits);
    }
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div>
          ...
          <input
            ...
            onChange={(e) => setBusinessSettings(prev => ({ ...prev, businessName:onChange={(e) => setBusinessSettings(prev => ({ ...prev, businessName: e.target.value }))}
